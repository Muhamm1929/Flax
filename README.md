...
- Профиль спрятан в кнопке-аватаре в хедере (как в мессенджерах), там же можно поменять имя.
- По клику на автора сообщения открывается профиль автора.
...
## Если Chrome показывает "сайт небезопасен / крадут данные"

Это обычно не баг логики чата, а репутация/настройка домена. Что сделать:

1. Использовать **только HTTPS** домен (в проекте добавлен авто-редирект HTTP -> HTTPS).
2. Поставить нормальный кастомный домен (не временный подозрительный URL).
3. В Vercel подключить домен и дождаться валидного SSL.
4. Не использовать фишинговые/подозрительные слова в поддомене.
5. Проверить Google Safe Browsing и запросить review, если домен помечен ошибочно.

Также в сервер добавлены security headers (CSP, HSTS, X-Frame-Options и др.), чтобы браузер меньше ругался.
